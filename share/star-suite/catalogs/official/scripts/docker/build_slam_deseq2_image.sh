...
if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
  printf '%s\n' \
    "Usage: $(basename "$0")" \
    "" \
    "Build and verify the pinned STAR Suite SLAM DESeq2 container." \
    "Configuration is supplied through IMAGE_TAG, BIOCONDUCTOR_IMAGE," \
    "BIOCONDUCTOR_VERSION, DESEQ2_VERSION, and TXIMPORT_VERSION."
  exit 0
fi
