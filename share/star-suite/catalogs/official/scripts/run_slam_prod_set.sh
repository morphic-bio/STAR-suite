...
# Production runner for a STAR-SLAM paired-end panel.
...
FASTQ_DIR="${FASTQ_DIR:-}"
OUT_BASE="${OUT_BASE:-${TMPDIR:-/tmp}/slam_pe_results/prod_run_${STAMP}}"
...
Default action: run paired-end STAR-SLAM per sample. Input and reference
locations must be supplied explicitly. Transfers are disabled unless both
endpoints and a destination root are supplied.
...
Paths:
  ...
  --snp-mask PATH            SLAM SNP mask BED.GZ; '-' disables it
...
Examples:
  ...
  # Run a selected panel with explicitly configured transfers.
  $(basename "$0") --panel '^treatment-' --globus-src-endpoint <source-id> \
    --globus-dst-endpoint <destination-id> --globus-dst-root <destination-path>
EOF
}
