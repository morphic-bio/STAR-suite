...
void Parameters::samAttributes(){//everything related to SAM attributes
    ...
    for (uint ii=0;ii<vAttr1.size();ii++) {
        ...
        } else if (vAttr1.at(ii)== "ZS") {
            outSAMattrOrder.push_back(ATTR_ZS);
            outSAMattrOrderQuant.push_back(ATTR_ZS);
            outSAMattrPresent.ZS=true;
        } else {
    ...
    samAttrRequiresBAM(outSAMattrPresent.ZS, "ZS");
