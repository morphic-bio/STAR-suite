...
using star::input::BinseqInputProbeModule;
...
struct HarnessOptions {
    ...
    std::string bqtools;
    std::string work_dir = "/tmp/star_suite_binseq_probe_harness";
...
void usage(std::ostream& out) {
    out << "Usage: binseq_probe_harness [options]\n"
        << "  --readFilesIn sample.cbq[,lane2.cbq]\n"
        << "  --readFilesManifest manifest.tsv  # CBQ<TAB>-<TAB>ReadGroup\n"
        << "  --bqtools PATH                     # default: $BQTOOLS or bqtools\n"
        << "  --workDir DIR                      # decode scratch directory\n"
        << "  --mateCount 1|2                    # records expected after decode\n"
...
HarnessOptions parse_args(int argc, char* argv[]) {
    HarnessOptions opts;
    const char* bqtools_env = std::getenv("BQTOOLS");
    opts.bqtools = (bqtools_env != nullptr && bqtools_env[0] != '\0') ? bqtools_env : "bqtools";
    ...
    for (int ii = 1; ii < argc; ++ii) {
        ...
        if (arg == "--help" || arg == "-h") {
            ...
            if (++ii >= argc) {
                throw std::runtime_error("--bqtools requires a value");
            }
            opts.bqtools = argv[ii];
        } else if (arg == "--workDir") {
            if (++ii >= argc) {
                throw std::runtime_error("--workDir requires a value");
            }
            opts.work_dir = argv[ii];
...
InputSourcePlan build_plan_from_manifest(const HarnessOptions& opts) {
    ...
    while (std::getline(manifest, line)) {
        ...
        if (fields.size() < 3) {
            throw std::runtime_error("BINSEQ manifest rows require CBQ<TAB>-<TAB>ReadGroup");
        }
        if (fields[1] != "-") {
            throw std::runtime_error("BINSEQ probe manifest second column must be '-' for paired CBQ records");
        }
    ...
    plan.command_string = opts.bqtools;
    plan.prefix = opts.work_dir;
