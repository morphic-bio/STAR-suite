...
bool flexPrepareCbqRangeTasks(FlexPipelineState *state, Parameters &P,
    ...
    // Spatial barcode fallback cost is highly non-uniform (for example, a
    // low-quality imaging tile can require the full edit-distance scan for
    // many adjacent reads).  A task-size ceiling bounds the serialized cost of
    // any one such region while preserving CBQ block locality and deterministic
    // global source ordinals.
    static const uint64_t kRecordsPerRange = 8192;
    const uint64_t chunkSize = std::min<uint64_t>(
        totalRecords, kRecordsPerRange);
