...
public:
    ...
    // Optional extras for downstream consumers
    int mapq;                // mapping quality (-1 if unset)
    std::string cigarString; // SAM-style CIGAR string (optional, may be empty)
    std::string chrName;     // Chromosome name (for probe reference mapping)
    int asScore;             // alignment score analogue (AS tag equivalent, if available)
    int nm;                  // mismatch count analogue (NM tag equivalent, if available)
