...
// Inline CB correction counters - now use thread-local counters via FlexDebugCounters.h
// Enable with -DDEBUG_FLEX_COUNTERS at compile time
#ifdef DEBUG_FLEX_COUNTERS
uint64_t getInlineCbExactCount() { return flexCountersSumAll().inlineCbExact; }
uint64_t getInlineCbCorrectedCount() { return flexCountersSumAll().inlineCbCorrected; }
uint64_t getInlineCbNRescuedCount() { return flexCountersSumAll().inlineCbNRescued; }
uint64_t getInlineCbRejectedCount() { return flexCountersSumAll().inlineCbRejected; }
uint64_t getInlineCbWithNCount() { return flexCountersSumAll().inlineCbWithN; }
#else
uint64_t getInlineCbExactCount() { return 0; }
uint64_t getInlineCbCorrectedCount() { return 0; }
uint64_t getInlineCbNRescuedCount() { return 0; }
uint64_t getInlineCbRejectedCount() { return 0; }
uint64_t getInlineCbWithNCount() { return 0; }
#endif
...
///////////////////////////SmartSeq  
    ...
    if (P.readFilesTypeN != 10) {//not SAM: barcode seq/qual are at the beginning of readNameExtra
        ...
        // Extract UR tag (raw UMI) for clique correction
        urSeq = "";
        urPacked = UINT32_MAX;
        urValid = false;
        size_t urPos = readNameExtraT.find("\tUR:Z:");
        if (urPos != std::string::npos) {
            urPos += 6; // Skip "\tUR:Z:"
            size_t urEnd = readNameExtraT.find('\t', urPos);
            if (urEnd == std::string::npos) {
                urEnd = readNameExtraT.length();
            }
            urSeq = readNameExtraT.substr(urPos, urEnd - urPos);
            // Validate UR length (should be 12bp for 10x)
            if (urSeq.length() == 12) {
                // Encode UR immediately
                urPacked = encodeUMI12(urSeq);
    ...
    if ( pSolo.type==pSolo.SoloTypes::CB_UMI_Simple ) {
        ...
        if (pSolo.CBtype.type==1) {//sequence cb
            ...
            if (pSolo.inlineCBCorrection) {
                ...
                bool nRescued = false;
                const bool cbHasN = cbSeq.find_first_of("Nn") != std::string::npos;

                if (cbHasN) {
                    FLEX_COUNT_INC(inlineCbWithN);
                }
                // If the CB contains Ns, allow a small N-rescue by substituting all Ns (up to 2)
                // and requiring a unique match; otherwise use the standard fast path.
                if (cbHasN) {
                    correctionResult = InlineCBCorrection::checkSequenceAndCorrectForN(cbSeq, /*maxN=*/2, correctedCB);
                    // Interpret a successful N rescue as a corrected match
                    if (correctionResult == 1) {
                        correctionResult = 1;
                        nRescued = true;
                    } else {
                        correctionResult = -1;
                    ...
                    correctionResult = InlineCBCorrection::fastPathCorrection(cbSeq, correctedCB);
                }
                
                // Debug: log CB correction failures (only for N-containing CBs to avoid noise)
                static int cbFailCount = 0;
                if (cbHasN && correctionResult != 0 && correctionResult != 1 && cbFailCount < 20) {
                    P.inOut->logMain << "SoloReadBarcode::getCBandUMI: CB correction FAILED - cbSeq=" << cbSeq
                                     << " (len=" << cbSeq.length() << "), correctionResult=" << correctionResult
                                     << ", correctedCB=" << (correctedCB.empty() ? "(empty)" : correctedCB) << std::endl;
                    cbFailCount++;
                }
                ...
                if (correctionResult == 0) {
                    // Exact match
                    cbMatch = 0;
                    cbSeqCorrected = correctedCB;
                    FLEX_COUNT_INC(inlineCbExact);
                    // Find whitelist index via exact hash
                    uint32_t idx1 = InlineCBCorrection::exactIndex(correctedCB);
                    if (idx1 > 0) {
                        cbMatchInd.push_back(static_cast<uint64_t>(idx1 - 1));
                        cbMatchString = std::to_string(cbMatchInd[0]);
                        InlineCBCorrection::recordParentEvidence(idx1);
                    } else if (cbFailCount < 20) {
                        P.inOut->logMain << "SoloReadBarcode::getCBandUMI: WARNING - correctedCB=" << correctedCB
                                         << " not found in whitelist (size=" << pSolo.cbWLstr.size() << ")" << std::endl;
                        cbFailCount++;
                        cbMatchString = "";
                    }
                    ...
                    cbSeqCorrected = correctedCB;
                    if (nRescued) {
                        FLEX_COUNT_INC(inlineCbNRescued);
                    } else {
                    ...
                    const uint64_t packed = InlineCBCorrection::packCBForLookup(cbSeq);
                    uint32_t resolvedIdx1 = InlineCBCorrection::resolveAmbiguousVariant(packed, cbSeq, cbQual, pSolo);
                    if (resolvedIdx1 > 0) {
                        // Treat as corrected via resolver
                        cbMatch = 1;
                        cbSeqCorrected = pSolo.cbWLstr[resolvedIdx1 - 1];
                        cbMatchInd.push_back(static_cast<uint64_t>(resolvedIdx1 - 1));
                        cbMatchString = std::to_string(cbMatchInd[0]);
                        InlineCBCorrection::recordParentEvidence(resolvedIdx1);
                    } else {
                        cbMatch = -1;
                        cbSeqCorrected = "-";
                        cbMatchInd.clear();
                        cbMatchString = "";
                        FLEX_COUNT_INC(inlineCbRejected);
                        // Capture ambiguous CB info for later Bayesian stub
                        if (InlineCBCorrection::isAmbiguousVariant(packed)) {
                            InlineCBCorrection::recordAmbiguousCB(packed, cbSeq, cbQual);
                        }
                    }
                }
                
                // Phase 1: Reject UR sequences containing N
                if (umiSeq.find('N') != std::string::npos || umiSeq.find('n') != std::string::npos) {
                    urValid = false;
                    urPacked = UINT32_MAX;
                } else {
                    // Encode UR if valid
                    urPacked = encodeUMI12(umiSeq);
                    urValid = (urPacked != UINT32_MAX);
                }
            } else {
...
// Inline CB correction counters (for debugging/validation)
