...
static void flexDecisionLedgerInit() {
    ...
    std::fprintf(g_flexDecisionLedger.fp,
        ...
        "\tambiguous_key_hex\tdetail\tprobe_hamming_distance\n");
    std::atexit([]() {
...
static void flexDecisionLedgerEvent(const char *event,
    ...
    const std::string probeHammingText = probeHammingDistance < 0
        ? "." : std::to_string(probeHammingDistance);

    std::lock_guard<std::mutex> lock(g_flexDecisionLedger.mutex);
    std::fprintf(g_flexDecisionLedger.fp,
        "%s\t%llu\t%s\t%lld\t%s\t%s\t%d\t%d\t%d\t%d\t%d\t%d"
        "\t%s\t%s\t%s\t%s\t%s\t.\t%s\t%d\t%s\t%d\t%s\t%s\t%s\t%s\t%s\t%s\n",
        event,
...
void flexDecisionLedgerTriage(uint64_t iRead, const char *qname,
    ...
    const std::string boundedName = qname == nullptr
        ? std::string() : std::string(qname, qnameLength);
    flexDecisionLedgerEvent("TRIAGE", iRead, boundedName.c_str(), lane, laneOrdinal,
                            ...
                            decision.probeHammingDistance == 0xFF
                                ? -1 : decision.probeHammingDistance);
}
...
static const char *flexCacheSourceName(uint8_t cacheClass)
{
    switch (cacheClass) {
...
void record_flex(SoloReadFeature *soloReadFeat, SoloReadBarcode &soloBar, uint nTr, Transcript **alignOut, uint64 iRead, ReadAnnotations &readAnnot)
{
    ...
    if (soloBar.cbMatch<0) {
        ...
        if (soloReadFeat->pSolo.inlineHashMode) {
            logRejectReason(soloBar, iRead, soloReadFeat->featureType, 0, 0, "NO_CB_MATCH", "", soloReadFeat->pSolo);
        }
        return;
    }
        ...
        switch (soloReadFeat->featureType) {
                    ...
                    if (readGe->size()==0) {//check genes
                        ...
                        if (debugNoGene) {
                            ...
                            std::fprintf(stderr,
                                ...
                                cbStr.c_str(),
                                (unsigned)nTr,
                                SoloFeatureTypes::Names[soloReadFeat->featureType].c_str());
                            
                            const auto &featAnnot = readAnnot.annotFeatures[soloReadFeat->featureType];
                        ...
                        // Log NO_GENE
                        if (soloReadFeat->pSolo.inlineHashMode) {
                            logRejectReason(soloBar, iRead, soloReadFeat->featureType, 0, 0, "NO_GENE", "", soloReadFeat->pSolo);
                        }
                        ...
                        if (soloReadFeat->pSolo.multiMap.yes.multi) {//output multimappers
                            ...
                            nFeat = outputReadCB_flex(soloReadFeat->streamReads, iRead, soloReadFeat->featureType, soloBar, reFe, readAnnot, soloReadFeat->readFlag, soloReadFeat);
                        } else {
                            // Multi-gene but multimappers disabled - drop the read
                            if (soloReadFeat->pSolo.inlineHashMode) {
                                logRejectReason(soloBar, iRead, soloReadFeat->featureType, 0, 0, "MULTIGENE_DROPPED", "", soloReadFeat->pSolo);
                            }
                        };
                    } else {//unique-gene reads
                        reFe.gene = *readGe->begin();
                        soloReadFeat->readFlag.setBit(soloReadFeat->readFlag.featureU);
                        nFeat = outputReadCB_flex(soloReadFeat->streamReads, (soloReadFeat->readIndexYes ? iRead : (uint64)-1), soloReadFeat->featureType, soloBar, reFe, readAnnot, soloReadFeat->readFlag, soloReadFeat);
                    };
            ...
            case SoloFeatureTypes::Velocyto :
                ...
                if (readAnnot.trVelocytoType.size()>0) {//otherwise, no gene
                    ...
                    if (!soloReadFeat->pSolo.inlineHashMode && soloReadFeat->streamReads) {
    ...
    if ( nFeat==0 && (soloReadFeat->readInfoYes | soloReadFeat->pSolo.readStatsYes[soloReadFeat->featureType]) ) {//no feature, but readInfo requested
        outputReadCB_flex(soloReadFeat->streamReads, iRead, (uint32)-1, soloBar, reFe, readAnnot, soloReadFeat->readFlag, soloReadFeat);
    };
