...
struct RawAlignment {
    ...
    uint32_t read_len;          // aligned read sequence length
    ...
    uint32_t mate_read_len;     // mate sequence length
    ...
    RawAlignment()
        : transcript_id(0), pos(0), score(0), est_aln_prob(0.0),
          ...
          is_forward(true), five_prime_pos(-1), read_len(0), mate_score(0), fragment_len(0),
          mate_is_forward(false), mate_pos(-1), mate_five_prime_pos(-1), mate_read_len(0), mate_fields_set(false),
          is_primary(true) {}
};
