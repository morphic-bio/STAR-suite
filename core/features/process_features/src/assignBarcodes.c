...
static int pf_lookup_strategy_inited = 0;
static pf_lookup_strategy_t pf_lookup_strategy = PF_STRATEGY_DEFAULT;

static pf_lookup_strategy_t pf_get_lookup_strategy(void) {
    if (!pf_lookup_strategy_inited) {
        const char *env = getenv("PF_LOOKUP_STRATEGY");
        if (env) {
            pf_lookup_strategy = (pf_lookup_strategy_t)atoi(env);
        }
        pf_lookup_strategy_inited = 1;
        if (pf_lookup_strategy != PF_STRATEGY_DEFAULT) {
            fprintf(stderr, "PF_LOOKUP_STRATEGY=%d\n", (int)pf_lookup_strategy);
        }
    }
...
static inline void hot_d0_insert(seq_hash_t *hot, seq_key_mode_t mode,
    ...
    if (mode == SEQ_KEY_64) {
        if (!hot->h64) return;
        uint64_t ikey = seq_encode_64_fixed(seq, slen);
        int ret;
        khint_t k = kh_put(u64u32, hot->h64, ikey, &ret);
        if (ret > 0) kh_val(hot->h64, k) = feat1;
    } else {
        if (!hot->h128) return;
        seq128_t ikey = seq_encode_128_fixed(seq, slen);
        int ret;
        khint_t k = kh_put(seq128u32, hot->h128, ikey, &ret);
        if (ret > 0) kh_val(hot->h128, k) = feat1;
    }
}
...
typedef enum {
    PF_RESOLVE_REASON_RESOLVED = 1,
    PF_RESOLVE_REASON_NO_HIT = 2,
    PF_RESOLVE_REASON_TOO_MANY_N = 3,
    PF_RESOLVE_REASON_AMBIGUOUS_TIE = 4,
    PF_RESOLVE_REASON_AMBIGUOUS_NO_FEATURE = 5
} pf_resolve_reason_t;

static inline void pf_stats_record_resolve(statistics *stats, int multi_alt, pf_resolve_reason_t reason) {
    if (!stats) {
        return;
    }
...
void initialize_statistics(statistics *stats) {
    ...
    stats->resolve_calls_total = 0;
    stats->resolve_calls_multi_alt = 0;
    stats->resolve_too_many_n = 0;
    stats->resolve_no_hit = 0;
    stats->resolve_ambiguous_tie = 0;
    stats->resolve_ambiguous_no_feature = 0;
    stats->resolve_resolved = 0;
    stats->resolve_multi_alt_too_many_n = 0;
    stats->resolve_multi_alt_no_hit = 0;
    stats->resolve_multi_alt_ambiguous_tie = 0;
    stats->resolve_multi_alt_ambiguous_no_feature = 0;
    stats->resolve_multi_alt_resolved = 0;
...
int simple_hash_search(feature_arrays *features, char *sequence){
    ...
    int result = feature_lookup_seq(sequence, feature_sequence_length);
...
static inline int adaptive_query_max_hamming(const feature_arrays *features,
                                             int max_hamming_distance,
                                             int best_hamming_distance,
                                             uint32_t best_feature) {
    int q = (best_hamming_distance <= max_hamming_distance) ? best_hamming_distance : max_hamming_distance;
    if (q < 0) q = 0;
    if (best_feature == 0 || !features) {
        return q;
    }
    if (q == 1 &&
        features->feature_no_ambiguity_le1 &&
        features->feature_no_ambiguity_le1[best_feature - 1]) {
        return 0;
    }
    if (q == 2 &&
        features->feature_no_ambiguity_le2 &&
        features->feature_no_ambiguity_le2[best_feature - 1]) {
        return 1;
    }
    return q;
}
...
int simple_hamming_search(feature_arrays *features, char *line, int maxHammingDistance, int *hamming_distance){  
    if (features->number_of_mismatched_features == 0 &&
        ...
        (int)strlen(line) >= features->common_length) {
        /* Policy: exact hash first, then <=1 cumulative hash, then <=2. */
        int exact_feature = simple_hash_search(features, line);
        if (exact_feature) {
            *hamming_distance = 0;
            return exact_feature;
        }
        ...
        uint32_t payload = 0;
        if (mode == SEQ_KEY_64) {
            uint64_t key = seq_encode_64_fixed(line, features->common_length);
            if (have_le1) {
                payload = seq_hash_get_64(&features->feature_hamming_le1_hash, key);
            }
            if (payload == 0 && have_le2 && maxHammingDistance >= 2) {
                payload = seq_hash_get_64(&features->feature_hamming_le2_hash, key);
            }
        } else {
            seq128_t key = seq_encode_128_fixed(line, features->common_length);
            if (have_le1) {
                payload = seq_hash_get_128(&features->feature_hamming_le1_hash, key);
            }
            if (payload == 0 && have_le2 && maxHammingDistance >= 2) {
                payload = seq_hash_get_128(&features->feature_hamming_le2_hash, key);
            }
        }

        if (maxHammingDistance >= 1 && have_le1) {
            if (payload && (((payload >> 29u) & 0x3u) <= 1u)) {
                const int payload_hamming = (int)((payload >> 29u) & 0x3u);
                const int payload_ambiguous = ((payload & 0x80000000u) != 0u);
                const int payload_feature = (int)(payload & 0x1FFFFFFFu);
                *hamming_distance = payload_hamming;
                if (!payload_ambiguous && payload_feature > 0 && payload_hamming <= maxHammingDistance) {
                    return payload_feature;
                }
                /* Ambiguous payload at best distance is terminal. */
                return 0;
            }
            if (maxHammingDistance == 1) {
                /* <=1 table is complete for this ceiling. */
                *hamming_distance = maxHammingDistance + 1;
                return 0;
            }
        }
        ...
        if (maxHammingDistance >= 2 && have_le2) {
            if (payload && (((payload >> 29u) & 0x3u) <= 2u)) {
                const int payload_hamming = (int)((payload >> 29u) & 0x3u);
                const int payload_ambiguous = ((payload & 0x80000000u) != 0u);
                const int payload_feature = (int)(payload & 0x1FFFFFFFu);
                *hamming_distance = payload_hamming;
            ...
            /* <=2 table is complete for this ceiling. */
            *hamming_distance = maxHammingDistance + 1;
            return 0;
        }
    }
...
int find_feature_match_parallel(feature_arrays *features, char *lineR2, int maxHammingDistance, int nThreads, int *bestScore, char **matching_sequence, uint16_t *match_position){
    ...
    for (int i=0; i<4; i++){
        ...
        int local_best_hamming=maxHammingDistance+1;
...
typedef struct {
    ...
    int hamming_distance;
    uint16_t match_position;
    const char *matching_sequence;
    char ambiguous;
} feature_match_candidate_t;

typedef void (*feature_match_eval_fn)(
    char *candidate,
    feature_arrays *features,
    int maxHammingDistance,
    int nThreads,
    ...
    int bestFeature,
    feature_match_candidate_t *out);

static void evaluate_feature_candidate_simple(
    char *candidate,
    feature_arrays *features,
    int maxHammingDistance,
    int nThreads,
...
{
    (void)nThreads;
    const int query_max_hamming = adaptive_query_max_hamming(
        features, maxHammingDistance, bestHammingDistance, (uint32_t)bestFeature);
    int myHammingDistance = query_max_hamming + 1;
    const int feature_index = simple_hamming_search(
        features, candidate, query_max_hamming, &myHammingDistance);

    out->feature_index = feature_index;
    out->hamming_distance = myHammingDistance;
    out->match_position = 0;
    out->matching_sequence = candidate;
    out->ambiguous = (feature_index == 0 && myHammingDistance <= query_max_hamming) ? 1 : 0;
}

static void evaluate_feature_candidate_parallel(
    char *candidate,
    feature_arrays *features,
    int maxHammingDistance,
    int nThreads,
    ...
    feature_match_candidate_t *out)
{
    (void)bestHammingDistance;
    (void)bestFeature;
    int myHammingDistance = maxHammingDistance + 1;
    char *myMatchingSequence = NULL;
    uint16_t myMatchPosition = 0;
    const int feature_index = find_feature_match_parallel(
        features, candidate, maxHammingDistance, nThreads,
        &myHammingDistance, &myMatchingSequence, &myMatchPosition);

    out->feature_index = feature_index;
    out->hamming_distance = myHammingDistance;
    out->match_position = myMatchPosition;
    out->matching_sequence = myMatchingSequence ? myMatchingSequence : candidate;
    out->ambiguous = (feature_index == 0 && myHammingDistance <= maxHammingDistance) ? 1 : 0;
}

static int resolve_feature_over_n_alts(
    char **corrected_seqs,
    int nAlts,
    feature_arrays *features,
    int maxHammingDistance,
    int nThreads,
    feature_match_eval_fn evaluator,
    feature_match_candidate_t *best_out,
    char *ambiguous_out)
{
    feature_match_candidate_t best = {
        .feature_index = 0,
        .hamming_distance = maxHammingDistance + 1,
        .match_position = 0,
        .matching_sequence = NULL,
        .ambiguous = 0
    };
    char localAmbiguous = 0;

    for (int i = 0; i < nAlts; i++) {
        feature_match_candidate_t current = {
            .feature_index = 0,
            .hamming_distance = maxHammingDistance + 1,
            .match_position = 0,
            .matching_sequence = NULL,
        ...
        if (current.feature_index && current.hamming_distance == 0) {
            best = current;
            localAmbiguous = 0;
            break;
        }

        if (current.hamming_distance < best.hamming_distance) {
            best = current;
            // Strictly better match resets ambiguity state.
            localAmbiguous = 0;
        } else if (current.hamming_distance == best.hamming_distance) {
            if (best.feature_index && current.feature_index) {
                if (best.feature_index != current.feature_index) {
                    localAmbiguous = 1;
                }
            } else if (best.feature_index && current.ambiguous) {
                localAmbiguous = 1;
            }
        }
    }
    ...
    if (ambiguous_out) {
        *ambiguous_out = localAmbiguous;
    }
    if (best.feature_index && !localAmbiguous && best.hamming_distance <= maxHammingDistance) {
        return best.feature_index;
    }
    return 0;
}
...
int simpleCorrectFeature(char *line, feature_arrays *features, int maxN, int maxHammingDistance, int *hamming_distance, statistics *stats){
    ...
    *hamming_distance = 0;
    ...
    if (nAlts <= 0) {
        *hamming_distance = maxHammingDistance + 1;
        pf_stats_record_resolve(stats, strchr(line, 'N') != NULL, PF_RESOLVE_REASON_TOO_MANY_N);
        return 0;
    }
    ...
    char myAmbiguous = 0;
    int resolved = resolve_feature_over_n_alts(
        corrected_seqs,
        nAlts,
        features,
        maxHammingDistance,
        1,
        evaluate_feature_candidate_simple,
        &best,
        &myAmbiguous);
    (void)myAmbiguous;
    *hamming_distance = best.hamming_distance;
    if (resolved) {
        pf_stats_record_resolve(stats, multi_alt, PF_RESOLVE_REASON_RESOLVED);
        return resolved;
    }
    if (myAmbiguous) {
        pf_stats_record_resolve(stats, multi_alt, PF_RESOLVE_REASON_AMBIGUOUS_TIE);
    } else if (best.hamming_distance <= maxHammingDistance) {
        pf_stats_record_resolve(stats, multi_alt, PF_RESOLVE_REASON_AMBIGUOUS_NO_FEATURE);
    } else {
        pf_stats_record_resolve(stats, multi_alt, PF_RESOLVE_REASON_NO_HIT);
    }
    return 0;
}
int checkAndCorrectFeature(char *line, feature_arrays *features,int maxHammingDistance, int nThreads, int *hamming_distance, char *matching_sequence, int maxN,char *ambiguous, uint16_t *match_position, statistics *stats){
    ...
    char myAmbiguous = 0;
    int feature_index = resolve_feature_over_n_alts(
        corrected_seqs,
        nAlts,
        features,
        maxHammingDistance,
        nThreads,
        evaluate_feature_candidate_parallel,
        &best,
        &myAmbiguous);
    ...
    *hamming_distance = best.hamming_distance;
    ...
    if (feature_index) {
        const size_t feature_len = (size_t)features->feature_lengths[feature_index - 1];
        const char *source = best.matching_sequence ? best.matching_sequence : line;
        memcpy(matching_sequence, source, feature_len);
        matching_sequence[feature_len] = '\0';
        if (match_position) {
            *match_position = best.match_position;
        }
        pf_stats_record_resolve(stats, multi_alt, PF_RESOLVE_REASON_RESOLVED);
    ...
    int *out_ambiguous)
{
    ...
    const int have_le2 = features->feature_hamming_le2_enabled;
    const seq_key_mode_t mode = features->code_hash_mode;
    const pf_lookup_strategy_t strategy = pf_get_lookup_strategy();
    ...
    *out_ambiguous = 0;
    ...
    if ((strategy >= PF_STRATEGY_HOT_D0 || use_hot_hash) && hot_d0) {
        ...
        if (mode == SEQ_KEY_64) {
            if (hot_d0->h64) {
                val = seq_hash_get_64(hot_d0, seq_encode_64_fixed(seq, feat_len));
            }
        } else if (hot_d0->h128) {
            val = seq_hash_get_128(hot_d0, seq_encode_128_fixed(seq, feat_len));
        }
        if (val && !(val & 0x80000000u)) {
            *out_hamming = 0;
            return (int)(val & 0x1FFFFFFFu);
        }
    }
    ...
    if (maxHammingDistance >= 1 && have_le1) {
        uint32_t payload = (mode == SEQ_KEY_64)
            ? seq_hash_get_64(&features->feature_hamming_le1_hash, seq_encode_64_fixed(seq, feat_len))
            : seq_hash_get_128(&features->feature_hamming_le1_hash, seq_encode_128_fixed(seq, feat_len));
        if (payload) {
    ...
    if (maxHammingDistance >= 2 && have_le2) {
        uint32_t payload = (mode == SEQ_KEY_64)
            ? seq_hash_get_64(&features->feature_hamming_le2_hash, seq_encode_64_fixed(seq, feat_len))
            : seq_hash_get_128(&features->feature_hamming_le2_hash, seq_encode_128_fixed(seq, feat_len));
        if (payload) {
            const int d = (int)((payload >> 29u) & 0x3u);
            const int amb = ((payload & 0x80000000u) != 0u);
            const int feat = (int)(payload & 0x1FFFFFFFu);
    ...
    uint16_t *out_match_position)
{
    ...
    for (int oi = 0; oi < n_offsets; oi++) {
        ...
        for (int a = 0; a < n_alts; a++) {
            ...
            int ambiguous = 0;
            int feat = pf_single_offset_hash_search(
                alt, feat_len, features, maxHammingDistance, hot_d0, &d, &ambiguous);
            if (feat > 0 && d < best_d) {
                best_feat = feat;
                best_d = d;
                best_pos = (uint16_t)off;
...
static void process_feature_sequence_internal(char *sequence, feature_arrays *features, int maxHammingDistance, int nThreads, int feature_constant_offset, int max_feature_n, uint32_t *feature_index, int *hamming_distance, char *matching_sequence, uint16_t *match_position, statistics *stats, seq_hash_t *hot_d0) {
    ...
    if (feature_mode_bootstrap_reads > 0 && features && features->feature_offsets &&
            ...
            for (int j = 0; j < features->number_of_features; j++) {
                ...
                for (int k = 0; k < 3; k++) {
                    ...
                    if (offset_cache_valid[current_offset]) {
                        ...
                        const int query_max_hamming = adaptive_query_max_hamming(features, maxHammingDistance, bestHamming, bestFeature);
                        idx = simpleCorrectFeature(sequence + current_offset, features, max_feature_n, query_max_hamming, &tmp_hamming, stats);
        ...
        if (!bestFeature && feature_mode_bootstrap_done == 0) {
            ...
            if (!bf_ambiguous && bf_feat > 0 && bf_hamming <= maxHammingDistance) {
                ...
                bestHamming = bf_hamming;
                bestPos = bf_match_pos;
                ambiguous = 0;
                ...
                for (int anchor_kind = 0; anchor_kind < PF_ANCHOR_KINDS; anchor_kind++) {
                    ...
                    while (anchor_pos >= 0) {
                        ...
                        if (bc_pos >= 0 && (size_t)bc_pos + feature_len <= read_len) {
                            int tmp_hamming = 0;
                            uint32_t idx = simpleCorrectFeature(sequence + bc_pos, features, max_feature_n, maxHammingDistance, &tmp_hamming, stats);
                            if (idx == (uint32_t)(j + 1)) {
                                *feature_index = idx;
    ...
    if (!use_feature_offset_array && limit_search != -1 && feature_constant_offset >= 0) {
        /* Build nearby candidate offsets in deterministic order: 0, +1, -1, +2, -2, ... */
        const int max_candidates = 2 * limit_search + 1;
        int candidate_offsets[max_candidates > 0 ? max_candidates : 1];
        int ncandidates = 0;
        for (int i = 0; i <= limit_search; i++) {
            for (int sign = 1; sign >= -1; sign -= 2) {
                ...
                int current_offset = feature_constant_offset + (i * sign);
                if (current_offset < 0 || current_offset >= (int)read_len) {
                    continue;
                }
                candidate_offsets[ncandidates++] = current_offset;
            }
        }

        if (feature_limited_fallback_mode == 1 && stats) {
            stats->limited_simple_fallback_calls++;
        }
        if (feature_limited_fallback_mode == 0 && stats) {
            stats->limited_full_fallback_calls++;
        }

        /*
         * Distance-phased policy over nearby offsets:
         * pass 0: exact hash at all candidate positions
         * pass 1: <=1 hash at all candidate positions (accept only d=1)
         * pass 2: <=2 hash at all candidate positions (accept only d=2)
         * ...
         */
        for (int d = 0; d <= maxHammingDistance; d++) {
            uint32_t pass_feature = 0;
            uint16_t pass_match_position = 0;
            int pass_ambiguous = 0;
            int pass_found = 0;
            for (int oi = 0; oi < ncandidates; oi++) {
                int current_offset = candidate_offsets[oi];
                int tmp_hamming = maxHammingDistance + 1;
                ...
                if (!pass_feature) {
                    pass_feature = idx;
                    pass_match_position = (uint16_t)current_offset;
                } else if (pass_feature != idx) {
                    pass_ambiguous = 1;
                }
            }
            ...
            if (pass_found) {
                if (!pass_ambiguous) {
                    if (feature_limited_fallback_mode == 1 && stats && d > 0) {
                        stats->limited_simple_fallback_hits++;
                    }
                    if (feature_limited_fallback_mode == 0 && stats && d > 0) {
                        stats->limited_full_fallback_hits++;
                    }
                    *feature_index = pass_feature;
                    *hamming_distance = d;
                    *match_position = pass_match_position;
                    memcpy(matching_sequence, sequence + pass_match_position, features->feature_lengths[pass_feature - 1]);
                    matching_sequence[features->feature_lengths[pass_feature - 1]] = '\0';
                } else {
                    *feature_index = 0;
                    *hamming_distance = d;
                }
    ...
    static __thread unsigned long long _trace_tiered_hit = 0;
    static __thread unsigned long long _trace_tiered_miss = 0;
    static __thread unsigned long long _trace_tiered_ambig = 0;
    ...
    if (!use_feature_offset_array && feature_constant_offset >= 0) {
        ...
        if (can_use_tiered) {
            ...
            if (minusOne_feature_constant_offset >= 0) {
                int minusOneFeatureIndex = simpleCorrectFeature(sequence + minusOne_feature_constant_offset, features, max_feature_n, maxHammingDistance, &constantHammingDistance, stats);
            ...
            if (plusOne_feature_constant_offset < (int)read_len) {
                int plusOneFeatureIndex = simpleCorrectFeature(sequence + plusOne_feature_constant_offset, features, max_feature_n, maxHammingDistance, &constantHammingDistance, stats);
    ...
    if (myFeatureIndex && bestHammingDistance) {
        ...
        int new_feature_index = checkAndCorrectFeature(sequence, features, variableMaxHammingDistance, nThreads, &variableHammingDistance, new_matching_sequence, max_feature_n, &ambiguous, &new_match_position, stats);
        ...
        int newFeatureIndex = checkAndCorrectFeature(sequence, features, variableMaxHammingDistance, nThreads, &variableHammingDistance, new_matching_sequence, max_feature_n, &ambiguous, &new_match_position, stats);
    ...
    *hamming_distance = bestHammingDistance;
    if ((_trace_total & 0x1FFF) == 0 && _trace_total > 0) {
        fprintf(stderr, "[TRACE tid=%d] total=%llu tiered_hit=%llu tiered_miss=%llu tiered_ambig=%llu legacy=%llu fallthrough=%llu le1_enabled=%d common_len=%d mismatched=%d\n",
                ...
                _trace_legacy, _trace_fallthrough,
                features ? features->feature_hamming_le1_enabled : -1,
                features ? features->common_length : -1,
                features ? features->number_of_mismatched_features : -1);
    }
}
...
void *consume_reads(void *arg) {
    ...
    if ((pf_get_lookup_strategy() >= PF_STRATEGY_HOT_D0 || use_hot_hash) &&
        ...
        features->number_of_mismatched_features == 0 &&
        features->common_length > 0) {
        seq_hash_init(&thread_hot_d0, features->code_hash_mode);
        if ((features->code_hash_mode == SEQ_KEY_64 && thread_hot_d0.h64) ||
            (features->code_hash_mode == SEQ_KEY_128 && thread_hot_d0.h128)) {
    ...
    while (!done) {
        ...
        for (int sweep = 0; sweep < nsets; sweep++) {
                            ...
                            for (int j = 0; j < features->number_of_features; j++) {
                                ...
                                for (int anchor_kind = 0; anchor_kind < PF_ANCHOR_KINDS; anchor_kind++) {
                                    ...
                                    while (anchor_pos >= 0) {
                                        ...
                                        uint32_t idx = 0;
                                        if (bc_pos >= 0 && (size_t)bc_pos + feature_len <= read_len) {
                                            idx = simpleCorrectFeature(feat_read + bc_pos, features, max_feature_n, maxHammingDistance, &tmp_hamming, stats);
                                        }
                                        fprintf(pf_trace_anchor_fp,
            ...
            if (thread_hot_d0_active && features && ((++hot_check_counter & 0x3FFFu) == 0u)) {
                khint_t hot_sz = seq_hash_size(&thread_hot_d0, features->code_hash_mode);
...
void merge_stats(statistics *merged_stats, statistics *thread_stats) {
    ...
    merged_stats->resolve_calls_total += thread_stats->resolve_calls_total;
    merged_stats->resolve_calls_multi_alt += thread_stats->resolve_calls_multi_alt;
    merged_stats->resolve_too_many_n += thread_stats->resolve_too_many_n;
    merged_stats->resolve_no_hit += thread_stats->resolve_no_hit;
    merged_stats->resolve_ambiguous_tie += thread_stats->resolve_ambiguous_tie;
    merged_stats->resolve_ambiguous_no_feature += thread_stats->resolve_ambiguous_no_feature;
    merged_stats->resolve_resolved += thread_stats->resolve_resolved;
    merged_stats->resolve_multi_alt_too_many_n += thread_stats->resolve_multi_alt_too_many_n;
    merged_stats->resolve_multi_alt_no_hit += thread_stats->resolve_multi_alt_no_hit;
    merged_stats->resolve_multi_alt_ambiguous_tie += thread_stats->resolve_multi_alt_ambiguous_tie;
    merged_stats->resolve_multi_alt_ambiguous_no_feature += thread_stats->resolve_multi_alt_ambiguous_no_feature;
    merged_stats->resolve_multi_alt_resolved += thread_stats->resolve_multi_alt_resolved;
}
...
int find_sample_index(char *seq, const sample_args *args, int *hamming){
    ...
    int idx = checkAndCorrectFeature(seq, args->sample_barcodes, args->sample_max_hamming, /*threads*/1, &hd, dummy_match, args->sample_max_N, NULL, &dummy_pos, NULL);
