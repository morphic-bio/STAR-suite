# TODO: Parallel FASTQ sharding in streams and in place
...
The headlining method is parallel sharding of FASTQ files for increased
processing performance. A caller can consume the result either as synchronized
shard streams or through independent in-place shard handles over the existing
file. An ordinary unindexed `fastq.gz` is the most important case because it is
a serial input bottleneck for otherwise parallel bioinformatics programs.
...
abstract should lead with parallel FASTQ sharding in streamed and in-place
forms. Present optional CBQ output only as a convenient demonstration that the
one unavoidable gzip pass can also leave an existing durable indexed
representation.
