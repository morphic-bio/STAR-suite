  ...
  - Completed:
    ...
    - Automated containerized install/uninstall validator added:
      ...
      - validates `apt install /path/to/star-suite*.deb`, binary presence,
        `STAR --version`, and `apt purge` cleanup on Ubuntu 22.04 and 24.04.
    - Tag-triggered releases build separate dynamic packages in the pinned
      Ubuntu 22.04 and 24.04 release-build containers. The 22.04 package is
      runtime-tested on both supported releases.
...
- Release tag: `v1.8.4`
- Release notes: `docs/RELEASE_NOTES_v1.8.4.md`
- Packaging metadata: `debian/changelog` entry `1.8.4-1`; GitHub binary
  packages `1.8.4-1~ubuntu22.04.1` and `1.8.4-1~ubuntu24.04.1`
- Scope: opt-in paired BGZF input for fused STAR-Flex coordinate-sorted BAM
  runs, using the existing STAR mapping and BAM-writing paths.
- Version note: release artifacts use tag `v1.8.4`, Debian packages and
  `STAR --version` report STAR Suite `1.8.4`, and both Ubuntu package variants
  come from the same tagged revision. Upstream provenance and genome-index
  compatibility remain `2.7.11b` and `2.7.4a`.

## Patch Release v1.8.3 (2026-09-05)

- Release tag: `v1.8.3`
- Release notes: `docs/RELEASE_NOTES_v1.8.3.md`
- Packaging metadata: `debian/changelog` entry `1.8.3-1`; GitHub binary
  packages `1.8.3-1~ubuntu22.04.1` and `1.8.3-1~ubuntu24.04.1`
- Scope: lower-copy fused BGZF and plain-gzip FASTQ ingest, direct packed CBQ
  probe and barcode lookup, and bounded parallel MEX output shared by fused
  Flex input modes.
- Version note: release artifacts use tag `v1.8.3`, Debian packages and
  `STAR --version` report STAR Suite `1.8.3`, and both Ubuntu package variants
  come from the same tagged revision. Upstream provenance and genome-index
  compatibility remain `2.7.11b` and `2.7.4a`.

## Patch Release v1.8.2 (2026-09-04)

- Release tag: `v1.8.2`
- Release notes: `docs/RELEASE_NOTES_v1.8.2.md`
- Packaging metadata: `debian/changelog` entry `1.8.2-1`; GitHub binary
  packages `1.8.2-1~ubuntu22.04.1` and `1.8.2-1~ubuntu24.04.1`
- Scope: fully fused Flex alignment progress, BGZF mate validation,
  STARsolo-compatible ambiguous-barcode posterior resolution, lower-contention
  fused-tail processing, and asynchronously consolidated sorted bucket runs.
- Version note: release artifacts use tag `v1.8.2`, Debian packages and
  `STAR --version` report STAR Suite `1.8.2`, and both Ubuntu package variants
  come from the same tagged revision. Upstream provenance and genome-index
  compatibility remain `2.7.11b` and `2.7.4a`.
...
- Dynamic (`amd64` GitHub release packages):
  - `star-suite_<version>-1~ubuntu22.04.1_amd64.deb`
  - `star-suite_<version>-1~ubuntu24.04.1_amd64.deb`
- Dynamic (PPA source builds):
  - architecture-native packages generated by Launchpad from
    `star-suite_<version>-1.dsc`
...
- The Ubuntu suffix identifies the dynamic package's build/test baseline, not
  a different STAR Suite feature set. Both packages are built from the same
  release commit and have the same output contract.
- The Ubuntu 22.04 package is validated on both Ubuntu 22.04 and 24.04. The
  Ubuntu 24.04 build may require newer runtime symbols and is not promised to
  run on Ubuntu 22.04.
