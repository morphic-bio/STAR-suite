#!/bin/bash
# Test sorted BAM with CB/UB tag injection
# Verifies the fix for the bitmask corruption bug in BAMoutput::coordBins()

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_DIR="${SCRIPT_DIR}/.."
OUT_DIR="${SCRIPT_DIR}/sorted_cbub_test_output"
TMP_DIR="/storage/100K/tmp/sorted_cbub_test"
STAR_BIN="${REPO_DIR}/core/legacy/source/STAR"

# Dataset and reference paths (same as regression test)
...
echo "=========================================="
echo "Sorted BAM CB/UB Tag Injection Test"
echo "=========================================="
echo ""
...
echo "Running STAR with sorted BAM + CB/UB tags..."
"$STAR_BIN" \
  --runThreadN 8 \
  --outTmpDir "$TMP_DIR" \
