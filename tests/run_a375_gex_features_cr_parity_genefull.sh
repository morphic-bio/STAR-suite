...
echo "=========================================="
echo "A375 GEX + Features CR-Parity Run (GeneFull)"
echo "=========================================="
...
# GEX will use EmptyDrops_CR filtering with GeneFull
# Features will automatically use GEX filtered barcodes via --soloCrGexFeature genefull
echo "Running STAR with GEX + Features (GeneFull mode)..."
echo "  GEX: CR-parity mode with EmptyDrops_CR filtering (GeneFull)"
...
# GEX outputs
GEX_RAW_MEX="${OUTPREFIX}Solo.out/GeneFull/raw"
GEX_FILTERED_MEX="${OUTPREFIX}Solo.out/GeneFull/filtered"
...
if [[ ! -d "${CR_MEX_DIR}" ]]; then
  ...
  echo "Comparing STAR GEX filtered (GeneFull) vs Cell Ranger filtered..."
