...
parser=argparse.ArgumentParser(description="Compare one ambiguity entry build against a preserved ledger")
...
src=args.source_dir.resolve() if args.source_dir else repo/'core/legacy/source'
...
cmd=['g++',*flags,*['-I'+str(p) for p in inc],str(repo/'tests/test_ambiguity_entry_storage.cpp'),'-Wl,--gc-sections','-pthread','-o',str(out/'probe')]
rec={'status':'building','commands':[cmd],'source_sha256':hashlib.sha256((src/'SoloReadFeature.h').read_bytes()).hexdigest(),'started_unix':time.time()};p=out/'execution.json'
