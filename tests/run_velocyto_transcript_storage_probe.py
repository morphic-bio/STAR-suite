...
parser=argparse.ArgumentParser(description="Compare one Velocyto transcript storage build against a preserved ledger")
parser.add_argument('--out', required=True, type=Path)
parser.add_argument('--source-dir', required=True, type=Path, help='Preserved baseline directory with SoloFeature.h and SoloFeature_countVelocytoBridge.cpp')
...
cmd=['g++',*flags,'-fopenmp',*['-I'+str(p) for p in inc],'-DBASELINE_SOURCE="'+str(src/'SoloFeature_countVelocytoBridge.cpp')+'"',str(repo/'tests/test_velocyto_transcript_storage.cpp'),'-Wl,--gc-sections','-pthread','-o',str(out/'probe')]
rec={'status':'building','commands':[cmd],'source_sha256':hashlib.sha256((repo/'core/legacy/source/PooledTranscriptMap.h').read_bytes()).hexdigest(),'started_unix':time.time()};p=out/'execution.json'
