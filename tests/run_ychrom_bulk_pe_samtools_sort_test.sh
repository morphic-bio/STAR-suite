#!/bin/bash
# Bulk Paired-End Y/NoY Split Validation Test with samtools sorter
# Tests --outBAMsortMethod samtools with Y/noY split

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
STAR_BIN="${SCRIPT_DIR}/../core/legacy/source/STAR"
BASE_DIR="${BASE_DIR:-/tmp/ychrom_bulk_pe_samtools_sort_test}"
REPORT_FILE="${SCRIPT_DIR}/TEST_REPORT_Y_SPLIT_BULK_PE_SAMTOOLS_SORT.md"
...
echo "=========================================="
echo "Bulk Paired-End Y/NoY Split Test (samtools sorter)"
echo "=========================================="
...
# Step 1: Baseline Run (no Y-split, for comparison)
...
"$STAR_BIN" \
    ...
    2>&1 | tee "$BASELINE_DIR/star.log" | grep -E "started|finished|ERROR|WARNING|samtools" || true

if [ ! -f "$BASELINE_DIR/Aligned.sortedByCoord.out.bam" ]; then
...
if command -v samtools &> /dev/null; then
    BASE_TOTAL=$(samtools view -c "$BASELINE_DIR/Aligned.sortedByCoord.out.bam" 2>/dev/null || echo "0")
    samtools index "$BASELINE_DIR/Aligned.sortedByCoord.out.bam" 2>/dev/null || true
    BASE_Y=$(samtools idxstats "$BASELINE_DIR/Aligned.sortedByCoord.out.bam" 2>/dev/null | awk '$1=="chrY" {y=$3+0} END {if (y=="") y=0; print y}')
    BASE_Y_DIRECT=$(samtools view "$BASELINE_DIR/Aligned.sortedByCoord.out.bam" 2>/dev/null | awk '$3=="chrY" {count++} END {print count+0}' || echo "0")
...
# Step 2: Sorted Split Run with samtools sorter
echo ">>> Step 2: Sorted Split Run (samtools sorter)"
...
"$STAR_BIN" \
    ...
    2>&1 | tee "$SORTED_DIR/star.log" | grep -E "started|finished|ERROR|WARNING|samtools" || true
...
validate_split() {
    ...
    echo "  Validating samtools-sorted split outputs..."
    ...
    # Store for report
    SORTED_Y_COUNT=$Y_COUNT
    SORTED_NOY_COUNT=$NOY_COUNT
}
...
# Step 4: Mate-checking verification
echo ">>> Step 4: Mate-Checking Verification (Bulk Paired-End)"
...
- Baseline BAM: \`${BASELINE_DIR}/Aligned.sortedByCoord.out.bam\`
