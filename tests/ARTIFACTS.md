...
- `/tmp/` or `tests/*_output*/` — outputs from `--runMode hashCacheGenerate` (`--hashCacheOutput` FH01SEQ1 binary); large; compare externally to pilot caches with `flex/tools/hash_screen_replay` if needed.
...
- `/storage/downsampled_100K/SC2300771/results/flex_h02_pilot_*` (Flex H2 two-mismatch pilot: synthetic FASTQ, STAR alignment, `h2_sequence_cache.tsv`, `h2_keep_only.bin`; merge sharded TSVs before final binary if `NUM_SHARDS>1`)
...
## Flex Internal Hash-Screen 100K E2E Rerun (2026-03-19)

- Harness: `tests/run_flex_hash_screen_internal_100k.sh`
- Cache: `/storage/downsampled_100K/SC2300771/results/flex_h01_full_cache_20260315_153914/reclassified/sequence_cache.bin`
- Completed run:
  - `/storage/downsampled_100K/SC2300771/results/flex_hash_screen_internal_20260319_235208/`
    - `hash_on/` current branch `benchmark-flex`
    - `legacy/` same-binary `--no-hash-screen yes` control
- Result:
  - raw pooled MEX parity failed
  - legacy raw: `43176` barcodes, `619448` nnz
  - hash_on raw: `43185` barcodes, `620176` nnz
  - filtered totals: legacy `12747`, hash_on `12756`
  - wall-clock: hash_on `0:51.12`, legacy `0:41.49`
- Status: untracked

## Flex Internal Hash-Screen 100K No-Clique Isolate (2026-03-20)

- Manual run surface: same as `tests/run_flex_hash_screen_internal_100k.sh`, but with explicit `--soloUMICorrection none`
- Completed run:
  - `/storage/downsampled_100K/SC2300771/results/flex_hash_screen_internal_noclique_20260320_000650/`
    - `hash_on/`
    - `legacy/`
- Important note:
  - required a local fix in `ParametersSolo.cpp` so `--flex yes` no longer overwrites an explicit `--soloUMICorrection none`
- Result:
  - logs confirm `soloUMICorrection=none` on both `hash_on` and `legacy`
  - raw pooled MEX parity still failed
  - hash-vs-legacy keyed raw mismatch remained `973` entries (`860` over, `113` under)
  - disabling clique changed only `9` keyed raw entries in `legacy` and `9` keyed raw entries in `hash_on`
- Status: untracked

## Hash Screen Replay Dump (2026-03-20)

- Generated by `STAR_DUMP_HASH_SCREEN` env var during 100K harness run
- Dump file: `/storage/downsampled_100K/SC2300771/results/flex_hash_screen_dump_20260320_005618/hash_screen_dump_v2.bin`
  - Format: HSCRN001 binary, 800K reads, 78 MB
  - Ground truth: 670,450 KEEP / 5,396 DENY / 124,154 PASS
- STAR output: `.../star_output_v2/`
- Replay validation: 0 mismatches (flat vs truth, tiered vs truth, flat vs tiered)
- Cache tier composition: H0=53,294 (1.3 MB) / H1=7,360,125 (176.6 MB) / deny=636,746 (15.3 MB) / dropped=19,879
- Replay tool: `flex/tools/hash_screen_replay/`
- Status: untracked
