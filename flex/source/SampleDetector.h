...
public:
    ...
    // exactOnly restricts the lookup to the supplied H0 table. Otherwise H1 is
    // queried after an H0 miss; *ambiguous is set when a tier has multiple owners.
...
private:
    ...
    // The supplied sequences form H0. H1 is constructed once by enumerating
    // all one-substitution variants; cross-owner variants are stored as
    // ambiguous. The two tiers are queried only at sampleProbeOffset.
    star::flex::SampleTagCache tagCache_;
