...
void SampleDetector::buildTagTable() {
    ...
    std::vector<uint16_t> tagOwners;
    const size_t nSamples = variantCountsPerSample_.size();
    for (size_t s = 0; s < nSamples; ++s) {
        for (uint8_t k = 0; k < variantCountsPerSample_[s]; ++k) {
            tagCodes.push_back(codeToIndex(sampleCodes_[s * 8 + k]));
            tagOwners.push_back(static_cast<uint16_t>(s + 1));
        }
    }
    tagCache_.build(tagCodes, tagOwners);
    const star::flex::SampleTagCache::Stats &stats = tagCache_.stats();
    tagStatExact_ = stats.exact;
    tagStatMismatch_ = stats.mismatch1;
    tagStatAmbiguous_ = stats.ambiguous;
    tagTableBuilt_ = true;
}
...
uint32_t SampleDetector::lookupIndex(uint16_t index, bool exactOnly, bool *ambiguous) const {
    ...
    const star::flex::SampleTagCache::Result result = tagCache_.lookup(index, allowMismatch);
    if (result.ambiguous) {
        if (ambiguous) *ambiguous = true;
        return 0u;
    }
    if (result.owner != 0u) {
        registerSampleToken(static_cast<uint8_t>(result.owner & 0x1Fu), result.owner);
    }
    return result.owner;
}
...
uint32_t SampleDetector::detectSampleIndex(const uint8_t *seqData, int32_t readLength, bool reverseStrand) const {
    ...
    const int pos = static_cast<int>(p_.sampleProbeOffset);
    if (pos < 0 || pos + 8 > readLength) return 0u;
    uint32_t code = 0;
    if (!reverseStrand) {
        for (int i = 0; i < 8; ++i) {
            const uint32_t nib = static_cast<uint32_t>(bam_seqi(seqData, pos + i) & 0xFu);
            if (!isACGTNib(nib)) return 0u;
            code = (code << 4) | nib;
        }
    } else {
        for (int i = 0; i < 8; ++i) {
            const int idx = pos + (7 - i);
            const uint32_t nib = static_cast<uint32_t>(bam_seqi(seqData, idx) & 0xFu);
            if (!isACGTNib(nib)) return 0u;
            const uint32_t comp = complementNib(nib);
            if (comp == 0u) return 0u;
            code = (code << 4) | comp;
        }
    }
