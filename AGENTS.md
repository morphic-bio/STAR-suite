...
- Do not assume similarly named GRCh38 reference roots use different STAR
  indices. On this installation, some wrapper roots symlink their `star/`
  directory to another reference.
- If reference provenance matters, check the actual `star/` symlink targets and
  `genomeParameters.txt` before drawing conclusions from parity results.
- Current local provenance to verify against `AGENTS.local.md`:
  - `/storage/autoindex_110_44/bulk_index` is the local harmonized build.
  - `/storage/autoindex_110_44/refdata-gex-GRCh38-autoindex11044-cellranger`
    wraps the same local build.
  - `/storage/autoindex_110_44/refdata-gex-GRCh38-autoindex11044-crstar`
    uses local FASTA/GTF but its `star/` directory resolves to the imported
    Cell Ranger index under `/storage/A375-CR-9.01/refdata-gex-GRCh38-2024-A/star`.
