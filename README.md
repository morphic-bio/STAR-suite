...
| Multiome ATAC | PBMC 3k 10x Multiome — STAR + concurrent libchromap + libMACS3 FRAG peaks + GEX `EmptyDrops_CR` + ATAC `evidence-from-peaks` (T=32, lease=256, pool=48, `--dynamicThreadFifoWaiters 1 --dynamicThreadAtacController 1`) | Cell Ranger ARC v2.2.0 same fixture (`cellranger-arc count --create-bam=true --nosecondary --disable-cell-annotation --localcores=32`); **40:04 (2404 s)** | **18:03 (1083 s); 2.22x faster** than ARC | Apples-to-apples scope: alignment + GEX UMI counting + GEX `EmptyDrops_CR` + ATAC mapping + libMACS3 FRAG peaks + ATAC per-barcode `peak_cutsites >= 1` cell call (the same set of stages ARC bundles inline). GEX `nCellsSimple` **2974** (Gene) / **2976** (GeneFull). Permit telemetry under FIFO + aggressive controller: max waiters 29, ATAC max wait **2.74 s**, MAP max wait **4.40 s**, 0 wait timeouts / 0 stall warnings. Integration guard fires when `--chromapAtacEnable 1 --dynamicThreadInterface 1` ends with `atacAcquire=0`. |
