...
Current production release: **STAR Suite v1.0.2**. The suite release tag and
packaging version are `v1.0.2` / `1.0.2-1`; `STAR --version` reports `1.0.2`.
...
| Multiome ATAC | PBMC 3k 10x Multiome benchmark; current production recipe uses one local STAR/Chromap pass for GEX `EmptyDrops_CR`, Velocyto, Y/noY output, concurrent low-memory Chromap ATAC, sorted BAM, and binary sidecar (`--chromapAtacSecondaryFragments star_out/atac_fragments.bin`), followed by native sidecar peak/MEX materialization with `star_multiome_atac_peak_mex` | Cell Ranger ARC v2.2.0 same fixture (`cellranger-arc count --create-bam=true --nosecondary --disable-cell-annotation --localcores=32`); **40:04 (2404 s)** | Historical integrated benchmark: **18:17.52 (1097.52 s); 2.19x faster** than ARC. Production JAX runs now use the sidecar post-materialization boundary documented in `docs/RUNBOOK_MULTIOME_MEX_MUDATA_20260516.md`. | Scope: alignment + GEX UMI counting + GEX `EmptyDrops_CR` + ATAC mapping + sidecar-derived peaks/peak MEX/ATAC metrics. The old in-STAR `chromapAtacMacs3FragPeaksSource` path is not the production recipe; ATAC metrics are derived from Chromap's fixed-record binary sidecar instead of re-reading gzipped fragments TSV. Sidecar validation: **53,969,811** records, **1,295,275,496 B**, `record_size=24`, `barcode_length=16`, **194** chrom rows. GEX `nCellsSimple` **2974** (Gene) / **2976** (GeneFull). |
