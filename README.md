...
| Multiome ATAC | PBMC 3k 10x Multiome (STAR + concurrent libchromap + libMACS3, T=32, lease=256, pool=48) | Same surface, `--dynamicThreadFifoWaiters 0` (legacy `notify_one` permit path, no controller) at pool=40 | **16:13** end-to-end with `--dynamicThreadFifoWaiters 1 --dynamicThreadAtacController 1`; **−11.4%** vs FIFO-off pool=40 (18:31) | ATAC permit max wait **2.78 s** / MAP max wait **4.80 s** (vs 6.05/6.12 s with FIFO alone at pool=40, and 27.5 s ATAC max wait at pool=40 with the legacy `notify_one` path); cross-domain wait gap within ~1–3% under matched FIFO settings; integration guard fails the run if `chromapAtacEnable=1` + `dynamicThreadInterface=1` ends with `atacAcquire=0` |
