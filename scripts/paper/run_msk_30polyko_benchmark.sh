...
REQUIRED_GB_NO_BAM="${MSK_REQUIRED_GB_NO_BAM:-80}"
REQUIRED_GB_WITH_BAM="${MSK_REQUIRED_GB_WITH_BAM:-350}"
...
require_free_gb() {
  ...
  if (( avail_gb < need_gb )); then
    ...
    echo "SOLUTION: choose a different --outdir on SSD with more space, or run --no-bam." >&2
    exit 1
  fi
}
