...
# The defaults match the 2026-05-11 SLAM PE smoke parameter surface.
...
Defaults:
  samples: ARID1A-no4su_S50, ARID1A-6h-1_S43
  output:  ${OUT_BASE}
...
Examples:
  $(basename "$0") --sample ARID1A-no4su_S50 --sample ARID1A-6h-1_S43
...
if ((${#REQUESTED_SAMPLES[@]} == 0)); then
  REQUESTED_SAMPLES=(ARID1A-no4su_S50 ARID1A-6h-1_S43)
fi
