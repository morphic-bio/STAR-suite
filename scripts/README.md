...
| `run_visium_hd_gex_sidecar_100k.py` | Python | Frozen source-only 100K Visium HD 3-prime GEX clean-room driver. Clean-builds STAR and companion tools, emits the post-rescue GeneFull ordinal sidecar without BAM/GX/UR, joins raw-R1 coordinate candidates and UMIs, runs GEX `MultiGeneUMI_CR`, repeats integer products for determinism, and materializes four policies at 2/8/16 micrometers. Writes `RUN_COMPLETE.json` only after all gates pass. See `docs/SPATIAL_FEATURE_SIDECAR.md`. |
