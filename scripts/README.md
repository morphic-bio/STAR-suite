...
| `run_visium_hd_gex_sidecar_100k.py` | Python | Frozen source-only 100K Visium HD 3-prime GEX clean-room driver. Clean-builds STAR and companion tools, runs the raw-R1/H0 and post-rescue GeneFull-sidecar producer branches concurrently within an explicit thread budget (with a serial control mode), joins them only after ordinal/digest completion gates, runs GEX `MultiGeneUMI_CR`, repeats integer products for determinism, and materializes four policies at 2/8/16 micrometers. Writes `RUN_COMPLETE.json` only after all gates pass. See `docs/SPATIAL_FEATURE_SIDECAR.md` and `docs/RUNBOOK_VISIUM_HD_GEX_CONCURRENT_PRODUCERS_20260722.md`. |
| `compare_visium_hd_spaceranger.py` | Python | Read-only descriptive sanity comparison between materialized Visium HD policy MEX directories and Space Ranger raw binned H5 matrices. Streams the large H5 arrays, validates feature/coordinate axes, and reports gene/bin overlap and correlations without feeding Space Ranger evidence back into resolution. |
...
| `integrate_feature_library.py` | Python | Merges a feature library (e.g., CRISPR guides) into existing downstream h5ads, adding per-library obs annotations. Supports `--ambient-fdr-calls-csv` to import ambient-FDR guide call support columns such as `guide_fdr_num_umis`, `guide_fdr_min_called_umi`, and `guide_fdr_min_qvalue`; also auto-discovers `ambient_fdr/guide_fdr_calls_per_cell.csv` next to compatibility `protospacer_calls_per_cell.csv` when present. |
