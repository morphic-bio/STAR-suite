...
| `run_ucsf_perturb_yremove_batch.sh` | Bash | Corrected-layout UCSF perturb production runner. Uses `STAR.release` by default, emits Y/noY BAM + FASTQ outputs, packages raw/filtered velocyto MEX, optionally runs downstream h5ad generation + CellBender, and prepares per-sample Globus transfer manifests/helpers. |
...
| `run_scrna_downstream_gene_full_velocyto.sh` | Bash | Downstream GeneFull + Velocyto packaging: builds `counts.h5ad`, filtered/singlet-filtered h5ads, integrates feature libraries, and optionally adds a CellBender denoised layer plus `final_counts.h5ad`. |
